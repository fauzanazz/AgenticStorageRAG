...
class TestDocumentsIntegration:
    ...
    async def test_list_pagination_validation(self, app, auth_token: str) -> None:
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            ...
            # page_size > 100
            resp = await client.get("/api/v1/documents?page_size=200", headers=headers)
            assert resp.status_code == 422

            # page_size < 1
            resp = await client.get("/api/v1/documents?page_size=0", headers=headers)
            assert resp.status_code == 422

    @pytest.mark.asyncio
    async def test_get_nonexistent_document(self, app, auth_token: str, mock_db: AsyncMock) -> None:
